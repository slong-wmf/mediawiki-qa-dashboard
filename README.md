...
The **Mobile / iOS Testing Dashboard** panel tracks
[`wikimedia/wikipedia-ios`](https://github.com/wikimedia/wikipedia-ios) GitHub
Actions test health. Live browser mode can fetch public run and artifact
metadata for `Run Unit Tests`, `Run UI Tests`, and `Run E2E Tests`. Static
snapshot mode reads `ios-testing.json`, generated by
`scripts/fetch-snapshot-data.js`, and can additionally download code coverage
metrics and result-bundle artifacts with failure videos using
`WIKIPEDIA_IOS_ARTIFACTS_TOKEN`.

For coverage, the snapshot parser reads the `*-coverage` artifact payloads and
the chart renders only the useful first-party targets:
...
Coverage and PR stability are computed over the latest 7-day window.
...
| `GITHUB_TOKEN` | Token used by `scripts/fetch-snapshot-data.js` for GitHub metadata fetches and higher API rate limits. In GitHub Actions this is the current repository's `${{ github.token }}`. | No |
| `WIKIPEDIA_IOS_ARTIFACTS_TOKEN` | Fine-grained token or GitHub App token with **Actions: read** on `wikimedia/wikipedia-ios`, used to download iOS coverage and result-bundle artifact ZIPs. | No — but required for iOS coverage deltas and inline UI recordings in static snapshot data |
...
- **iOS coverage and recordings** require authenticated GitHub Actions artifact
  downloads from `wikimedia/wikipedia-ios`. Live browser mode can show public
  run/artifact metadata but cannot download artifact ZIPs, so iOS coverage
...
toolforge envvars create WIKIPEDIA_IOS_ARTIFACTS_TOKEN  # Actions: read on wikimedia/wikipedia-ios
