...
`scripts/fetch-snapshot-data.js`, and can additionally download
code coverage metrics and result-bundle artifacts with failure videos using the `GITHUB_TOKEN`.
...
| `GITHUB_TOKEN` | Token used by `scripts/fetch-snapshot-data.js` to download GitHub Actions coverage and result-bundle artifacts for iOS snapshots | No — but required for iOS coverage deltas and inline UI recordings in static snapshot data |
...
- **iOS coverage and recordings** require authenticated GitHub Actions artifact
  downloads. Live browser mode can show public run/artifact metadata but cannot
  download artifact ZIPs, so iOS coverage deltas and inline failed-run
  recordings are populated by static snapshot data generated with `GITHUB_TOKEN`.
